...
установки. Необязательное поле `FIRECRAWL_API_URL` в диалоге установки — адрес Firecrawl API;
по умолчанию официальный, менять только под свой прокси или ротатор ключей. Внешние бинарники: `jq`, `uv`, `pdftotext` из poppler, опционально `yt-dlp`. Только
